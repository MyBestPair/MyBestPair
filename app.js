...
function budgetCompat(price,budget){
  ...
  return Math.max(
    0,
    80-((price-(budget+FLEX_BUDGET))/FLEX_BUDGET)*40
  );
}
...
function compute(shoe,p){
  ...
  const brand=
    p.brand!=="AUCUNE" &&
    stripAccentsUpper(shoe.scoreBrand)===stripAccentsUpper(p.brand)
      ? 100
      : 0;
  ...
  const priorityNotes=p.priorities.map(
    c=>shoe.scores[CRITERIA.indexOf(c)]
  );

  const avg=
    priorityNotes.reduce((a,b)=>a+b,0)/3;
  ...
  return `
    <article class="shoe-card ${klass}">
      ...
      <div class="card-top">
        ...
        <div class="score">
          ⭐ Score MYSPORTSHOES : ${Math.round(r.final)} / 100
        </div>
      ...
      <div class="card-body">
        ...
        <div class="info-grid">
          ...
          <div class="info">
            💰 <strong>${euro(r.price)}</strong> · ${r.budgetStatus.text}
          </div>

          <div class="info">
            🏠 Surface :
            <strong>${surfaceText(r.surface)}</strong>
            · ${surfaceFit(r.surfaceCompat)}
          </div>

          <div class="info">
            👟 Pied :
            <strong>${footText(r.footCompat)}</strong>
          </div>

          <div class="info">
            🏷️ Marque :
            <strong>${r.brand.toUpperCase()}</strong>
          </div>

          <div class="info">
            ⭐ Affinité marque :
            <strong>${brandText(r.brandCompat)}</strong>
          </div>
        ...
        <div class="analysis">
          ...
          <div class="watch ${r.watch.balanced?"balanced":""}">
            ${r.watch.text}
          </div>

          <div class="why">
            <strong>💡 Pourquoi cette chaussure ?</strong><br>
            ${r.why}
          </div>
...
function render(){
  ...
  $("more-btn").hidden=
    shownCount>=Math.min(6,rankedResults.length);
}
...
document.addEventListener("click",e=>{
  ...
  /*
   * Les éléments "Lien bientôt disponible" utilisent aussi
   * la classe product-link mais ne sont pas de vrais liens.
   */
  ...
  if(card?.classList.contains("rank1"))
    rank=1;

  else if(card?.classList.contains("rank2"))
    rank=2;

  else if(card?.classList.contains("rank3"))
    rank=3;
  ...
  if(typeof gtag==="function"){
     ...
     */
    const isRakuten=
      href.includes("click.linksynergy.com");
    ...
    /*
     * Événement envoyé pour TOUS les clics vers une fiche produit.
     */
    ...
    /*
     * Événement supplémentaire uniquement lorsqu'il s'agit
     * d'un lien affilié.
     */
    if(isAffiliate){
...
$("profile-form").addEventListener("submit",e=>{
  ...
  if(new Set(p.priorities).size!==3)
    return;
  ...
  rankedResults=
    SHOES
      ...
      .sort((a,b)=>{
        ...
        /*
         * Quand deux modèles sont pratiquement à égalité
         * (< 0,10 point), on privilégie d'abord la meilleure
         * compatibilité budget.
         *
         * Cela évite qu'une paire hors budget passe devant
         * une paire dans le budget pour quelques centièmes
         * invisibles à l'écran.
         */

        if(Math.abs(gap)<0.10){

          if(b.budgetCompat!==a.budgetCompat)
            return b.budgetCompat-a.budgetCompat;

          if(b.surfaceCompat!==a.surfaceCompat)
            return b.surfaceCompat-a.surfaceCompat;

          if(b.footCompat!==a.footCompat)
            return b.footCompat-a.footCompat;
  ...
  /*
   * ANALYTICS :
   * questionnaire terminé et recommandations générées.
   */
